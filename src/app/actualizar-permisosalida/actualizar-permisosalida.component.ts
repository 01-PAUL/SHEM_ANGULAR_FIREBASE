...
import { PermisoSalida } from '../model/permisosalida.model';
import jsPDF from 'jspdf';
import * as XLSX from 'xlsx';

@Component({
  selector: 'app-actualizar-permisosalida',
  standalone: true,
  imports: [
        AppMaterialModule,
        CommonModule,
  ...
  templateUrl: './actualizar-permisosalida.component.html',
  styleUrl: './actualizar-permisosalida.component.css'
})
export class ActualizarPermisosalidaComponent {
  permisoSalida: { key: string; data: PermisoSalida }[] = [];
  permisoSalidaFiltrados: { key: string; data: PermisoSalida }[] = [];
  filtroTexto: string = '';
  ...
  ngOnInit(): void {
    this.http
      .get<{ [key: string]: PermisoSalida }>(
        'https://shem-firebase-default-rtdb.firebaseio.com/Permiso Salida.json'
      )
      .subscribe((data) => {
        if (data) {
          this.permisoSalida = Object.entries(data)
          ...
          this.permisoSalidaFiltrados = [...this.permisoSalida]; 
        }
      });
  }
  ...
  validarEmoji(): void {
    ...
    setTimeout(() => {
      if (emojiRegex.test(this.filtroTexto)) {
        ...
        this.permisoSalidaFiltrados = [...this.permisoSalida]; // Restablece la lista de estudiantes
      }
    }, 0); 
  }
  ...
  buscarIngreso(): void {
    ...
    if (filtro) {
      this.permisoSalidaFiltrados = this.permisoSalida.filter((permisoSalida) =>
        permisoSalida.data.area.toLowerCase().includes(filtro) 
      );
    } else {
      this.permisoSalidaFiltrados = [...this.permisoSalida];  
    }
  ...
  async descargarPDF(): Promise<void> {
    ...
    const titulo = 'Reporte de Permiso Salida de Usuarios';
    doc.setFontSize(35);
    ...
    const columnas = [
      ...
      'Fecha Salida', 'Hora Salida', 'Tipo Micromovilidad', 'Imagen'
    ];
  
    // Genera las filas con las imágenes convertidas
    const filas = [];
    for (const permisoSalida of this.permisoSalidaFiltrados) {
      const imagenBase64 = await this.convertImageToBase64(permisoSalida.data.imageUrl);
      filas.push([
        permisoSalida.data.usuario,
        permisoSalida.data.tipoDocumento,
        permisoSalida.data.numeroDocumento,
        permisoSalida.data.area,
        permisoSalida.data.detallePermiso,
        permisoSalida.data.fechaSalida,
        permisoSalida.data.horaSalida,
        permisoSalida.data.micromovilidad,
  ...
  const cabeceras = [
    'USUARIO', 'TIPO DOCUMENTO', 'N° DOCUMENTO', 'ÁREA', 'DETALLE PERMISO', 'FECHA SALIDA', 'HORA SALIDA', 'TIPO MICROMOVILIDAD', 'IMAGEN DEL USUARIO'
  ];

  // Mapea los datos a exportar
  const datos = this.permisoSalidaFiltrados.map(permisoSalida => ({
    'USUARIO': permisoSalida.data.usuario,
    'TIPO DOCUMENTO': permisoSalida.data.tipoDocumento,
    'N° DOCUMENTO': permisoSalida.data.numeroDocumento,
    'ÁREA': permisoSalida.data.area,
    'DETALLE PERMISO': permisoSalida.data.detallePermiso,
    'FECHA SALIDA': permisoSalida.data.fechaSalida,
    'HORA SALIDA': permisoSalida.data.horaSalida,
    'TIPO MICROMOVILIDAD': permisoSalida.data.micromovilidad,
    'IMAGEN DEL USUARIO': permisoSalida.data.imageUrl ? permisoSalida.data.imageUrl : 'No disponible',
  }));
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'PermisoSalida');

  // Escribe y descarga el archivo Excel
  XLSX.writeFile(workbook, 'Listado_PermisoSalida.xlsx');
}
